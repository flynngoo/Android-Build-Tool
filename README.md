...
- **设计系统**：采用 **Neumorphism (Soft UI)** 设计系统，具有以下特点：
  - **3D 深度效果**：使用双阴影（左上光源 + 右下阴影）创建物理深度感
  - **单色背景**：Cool Grey (#E0E5EC) 作为基础表面，所有元素都从同一材质"塑造"而成
  - **圆润边角**：32px 容器圆角、16px 按钮圆角，营造柔和、友好的视觉
  - **凸出/凹陷状态**：元素可以凸出（Extruded）或凹陷（Inset），创造真实的物理交互感
  - **平滑过渡**：300ms 过渡动画，提供流畅的微交互体验
  - **可访问性**：WCAG AA 对比度标准，清晰的焦点状态，44px+ 触摸目标
...
- **多架构构建说明**：
  ...
  - **为不同架构构建**：
    ...
    - **构建通用二进制文件（Universal Binary）**（同时包含两种架构）：
      ...
      ```
- **功能**：环境检测、工程列表/添加/编辑/删除（写入 `config/projects.json`）、选择工程/模块/variant 进行构建并展示日志、自动发布到蒲公英/fir.im、构建成功后一键打开输出文件夹。
